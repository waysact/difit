...
Treat `Review went idle, shutting down server...` without comments as a successful no-comments outcome.
...
```bash
pnpm run dev <target> [compare-with] --title "Fix auth timeout bug"
```
